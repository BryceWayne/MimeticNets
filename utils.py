...
def accuracy(model, dataset_loader, device, repeat=1, save_adv=None, criterion=None, attack=None, binarize=False, modelM=None, N=1, div=1, grad=1) :
    
    def DE(x, g, div, grad):
        batch = x.shape[0]
        x = x.reshape(batch, -1)
        g = g*torch.mm(grad, x.T)
        return (x + (div@g).T).reshape(batch, 1, 28, 28)
