...
from model.mimetic import Net


def adv_train_module(attack, model, data_type, iters, device, modelM) :
    """
    ...
    if attack == None :
        ...
        adv = LinfPGD(model, bound=bound, step=step, iters=iters, norm=norm, random_start=random_start, device=device, modelM=modelM)
    elif attack == "fgsm" :
        adv = FGSM(model, bound=bound, norm=norm, random_start=random_start, device=device, modelM=modelM)
    elif attack == "ball" :
...
def mimetic_trainer(model, logger, loader, args, data="mnist", optimizer=None, scheduler=None, adv_train=None, tboard=True, modelM=None, **kwargs) :
    ...
    adv, stats = adv_train_module(adv_train, model, data, args.iters, device, modelM)

    # torch.save({"state_dict": model.state_dict(), "args": args}, os.path.join(args.save, "model_acc.pt"))
    torch.save({"state_dict": modelM.state_dict(), "args": args}, os.path.join(args.save, "modelM_acc.pt"))
    # torch.save({"state_dict": model.state_dict(), "args": args}, os.path.join(args.save, "model_loss.pt"))
    torch.save({"state_dict": modelM.state_dict(), "args": args}, os.path.join(args.save, "modelM_loss.pt"))
    for itr in tqdm.tqdm(range(args.epochs * batches_per_epoch)) :
        ...
        if adv_train is not None :
            x = adv.perturb(x,y,device=device)
            if adv_train == "ball" :
        ...
        if itr % batches_per_epoch == 0 :
            ...
            modelM.eval()
            with torch.no_grad() :
                train_acc, train_loss = accuracy(model, dataset_loader=loader['train_eval_loader'], device=device, criterion=criterion, modelM=modelM, N=args.N, div=div, grad=grad)
                val_acc, val_loss = accuracy(model, dataset_loader=loader['test_loader'], device=device, criterion=criterion, modelM=modelM, N=args.N, div=div, grad=grad)
                if val_acc >= best_acc :
                    # torch.save({"state_dict": model.state_dict(), "args": args}, os.path.join(args.save, "model_acc.pt"))
                    torch.save({"state_dict": modelM.state_dict(), "args": args}, os.path.join(args.save, "modelM_acc.pt"))
                    best_acc = val_acc
            ...
            torch.save({"state_dict": modelM.state_dict(), "args": args}, os.path.join(args.save, "modelM_final.pt"))
            ind = int(itr//batches_per_epoch)
            data_to_save[ind] = {'val_acc':val_acc, 'best_acc':best_acc}
    ...
    import pickle
    with open(os.path.join(args.save, 'data_to_save.pickle'), 'wb') as handle:
        pickle.dump(data_to_save, handle, protocol=pickle.HIGHEST_PROTOCOL)
        
    old_model = torch.load(os.path.join(args.save, "modelM_acc.pt"))
    modelM.load_state_dict(old_model['state_dict'])
    model.eval()
    return modelM, data_to_save

def test(model, target_loader, device, modelM, N, **kwargs) :
    modelM.eval()
    # modelM = None
    # N = 0
    model.eval()
    ...
    div = torch.from_numpy(  DIV2D(2, 26, 1, 26, 1)).float().to(device)
    with torch.no_grad() :
        eval_acc, eval_loss = accuracy(model, dataset_loader=target_loader, device=device, criterion=nn.CrossEntropyLoss().to(device), modelM=modelM, N=N, div=div, grad=grad)
    return eval_acc, eval_loss

# # The below function is not used in our experiment.
# def adversarial_attack(model, logger, target_loader, save_adv, args, modelM, **kwargs) :
#     logger.info("="*80)
